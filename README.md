...
⚠️ **Note**: RecMpox is primarily designed to investigate potential recombination between viruses circulating in sustained human outbreaks (for example, SH2017, SH2023b, and SH2024a). The reference genomes provided by default in the tool correspond to these sustained outbreak lineages. When applying RecMpox outside this context, select reference genomes that are genetically close to your consensus sequences, or alternatively use the optional **-phylogeny** mode to confirm that the two tracts cluster with two different outbreaks.
